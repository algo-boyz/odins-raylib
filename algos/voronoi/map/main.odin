...
screen_ratio := [2]f32 {
    WIDTH < HEIGHT ? 1.0 : (f32(WIDTH) / f32(HEIGHT)),
    WIDTH > HEIGHT ? 1.0 : (f32(HEIGHT) / f32(WIDTH)),
}
...
draw_shadowed_heightmap :: proc() {
    ...
    source_rect := rl.Rectangle{0, 0, f32(WIDTH), -f32(HEIGHT)}
    rl.DrawTextureRec(heightmap_rt.texture, source_rect, rl.Vector2{0, 0}, rl.WHITE)
    rl.EndShaderMode()
}
...
update_sun :: proc() {
    ...
    sun_pos := rl.Vector3{mouse_pos.x / f32(WIDTH), mouse_pos.y / f32(HEIGHT), 1.0}
...
main :: proc() {
    ...
    rl.InitWindow(WIDTH, HEIGHT, "Heightmap")
    rl.SetTargetFPS(60)

    // Init the shaders and textures
    heightmap_sh = rl.LoadShader("assets/shader.vs", "assets/heightmap.fs")
    heightmap_rt = rl.LoadRenderTexture(WIDTH, HEIGHT)
