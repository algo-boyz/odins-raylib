...
MoveObject :: struct {
    pos: sh.Point, velocity: sh.Point, speed: f32, settled: bool, idle_timer: f32,
    target_pos: sh.Point, has_target: bool, trail: [20]sh.Point, trail_index: int,
}
g_map: []GridInfo
g_map_w, g_map_h: i32
...
resolve_unit_obstacle :: proc(obj: ^MoveObject, old_pos: sh.Point) {
...
main :: proc() {
    ...
    defer delete(g_map)
    defer sh.destroy(&g_spatial_hash)
    rl.SetTargetFPS(60)
    initial()
...
separation_force :: proc(i: int, sep_dist: f32 = 5.0, strength: f32 = 0.3) -> sh.Point {
    sep: sh.Point = {0, 0}
    count := 0
    d2 := sep_dist * sep_dist
    nearby: [dynamic]int
    defer delete(nearby)
    sh.query(&g_spatial_hash, g_object_positions[i].pos, sep_dist * 2, &nearby)
    for j in nearby {
        if i == j { continue }
...
cohesion_force :: proc(i: int, coh_dist: f32 = 8.0, strength: f32 = 0.2) -> sh.Point {
    coh: sh.Point = {0, 0}
    count := 0
    d2 := coh_dist * coh_dist
    nearby: [dynamic]int
    defer delete(nearby)
    sh.query(&g_spatial_hash, g_object_positions[i].pos, coh_dist * 2, &nearby)
    for j in nearby {
        if i == j { continue }
...
alignment_force :: proc(i: int, align_dist: f32 = 6.0, strength: f32 = 0.25) -> sh.Point {
    align: sh.Point = {0, 0}
    count := 0
    d2 := align_dist * align_dist
    nearby: [dynamic]int
    defer delete(nearby)
    sh.query(&g_spatial_hash, g_object_positions[i].pos, align_dist * 2, &nearby)
    for j in nearby {
        if i == j { continue }
        dx := g_object_positions[j].pos.x - g_object_positions[i].pos.x
        dy := g_object_positions[j].pos.y - g_object_positions[i].pos.y
...
obstacle_repulsion_force :: proc(i: int, rep_dist: f32 = 6.0, rep_strength: f32 = 1.5) -> sh.Point {
...
update :: proc() {
    ...
    if rl.IsMouseButtonPressed(.LEFT) {
        ...
        ci := world_to_index(pixel_to_world(sh.Point{f32(click_up_x), f32(click_up_y)}))
...
mouse_move :: proc() {
    ...
    x := i32(mouse_pos.x)
    y := i32(mouse_pos.y)
    d_idx := world_to_index(pixel_to_world(sh.Point{f32(x), f32(y)}))
    draw_obs := false
...
change_object_position :: proc() {
    dt := rl.GetFrameTime()
    sh.clear(&g_spatial_hash)
    for i in 0..<MOVE_OBJECT_NUM { sh.insert(&g_spatial_hash, g_object_positions[i].pos, i) }
    settled_count := 0
    ...
    for i in 0..<MOVE_OBJECT_NUM {
        ...
        if dist_to_dest < current_arrival_radius {
            if !obj.has_target {
                ...
                for attempt in 0..<100 {
                    ...
                    test_idx := world_to_index(sh.Point{test_x, test_y})
                    ...
                    nearby: [dynamic]int
                    defer delete(nearby)
                    sh.query(&g_spatial_hash, sh.Point{test_x, test_y}, 2 * UNIT_RADIUS, &nearby)
                    for j in nearby {
                        if i == j { continue }
                    ...
                    if score > best_score {
                        best_score = score
                        best_pos = sh.Point{test_x, test_y}
            ...
            if dist < 0.5 {
                ...
                if obj.settled {
                    ...
                    local_center: sh.Point = dest_world  // Fallback to dest
                    coh_count := 0
                    nearby: [dynamic]int
                    defer delete(nearby)
                    sh.query(&g_spatial_hash, obj.pos, DEST_CLUSTER_RANGE * 2, &nearby)
                    for j in nearby {
                        if i == j { continue }
                        other := &g_object_positions[j]
                        if !other.settled { continue }
                    ...
                    coh_dir := sh.Point{local_center.x - obj.pos.x, local_center.y - obj.pos.y}
                    ...
                    ali := sh.Point{0, 0}  // No alignment in blob

                    // Weak pull to global dest to prevent drift
                    dest_pull := sh.Point{dest_world.x - obj.pos.x, dest_world.y - obj.pos.y}
        ...
        // flow bias
        if has_flow {
            target := sh.Point{flow_dir.x * obj.speed, flow_dir.y * obj.speed}
            blend :f32 = 0.5
...
resolve_collisions :: proc() {
    // Rebuild hash for post-move positions
    sh.clear(&g_spatial_hash)
    for i in 0..<MOVE_OBJECT_NUM { sh.insert(&g_spatial_hash, g_object_positions[i].pos, i) }
    ...
    for pass in 0..<3 {
        for i in 0..<MOVE_OBJECT_NUM {
            ...
            nearby: [dynamic]int
            defer delete(nearby)
            sh.query(&g_spatial_hash, pos_i, 2 * UNIT_RADIUS, &nearby)
            for j in nearby {
        ...
        // Re-hash after pass for next iteration
        sh.clear(&g_spatial_hash)
        for k in 0..<MOVE_OBJECT_NUM { sh.insert(&g_spatial_hash, g_object_positions[k].pos, k) }
    }

    // Damp velocities if big displacements happened to prevent jitter
    for &obj in g_object_positions {
        obj.velocity.x *= 0.95
        obj.velocity.y *= 0.95
    }
...
draw_map :: proc() {
    ...
    for y in 0..<g_map_h {
        for x in 0..<g_map_w {
            ...
            rl.DrawRectangleRec(rl.Rectangle{p1.x * scale_x, p1.y * scale_y, GRID_SIZE * scale_x, GRID_SIZE * scale_y}, color)
...
draw_destination :: proc(p: sh.Point) {
    scale_x := f32(g_cur_win_width) / WORLD_WIDTH
    scale_y := f32(g_cur_win_height) / WORLD_HEIGHT
...
draw_flow_field :: proc() {
    ...
    for y in 0..<g_map_h {
        for x in 0..<g_map_w {
            ...
            ep := sh.Point{sp.x + dir.x * GRID_SIZE/2, sp.y + dir.y * GRID_SIZE/2}
...
draw_cost_overlay :: proc() {
    ...
    for y in 0..<g_map_h {
        for x in 0..<g_map_w {
            ...
            rl.DrawRectangleRec(rl.Rectangle{p.x * scale_x, p.y * scale_y, GRID_SIZE * scale_x, GRID_SIZE * scale_y}, col)
        }
    }
}
