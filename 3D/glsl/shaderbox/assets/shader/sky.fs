...
// Port the original shader functions
mat2 mm2(in float a) {
    float c = cos(a), s = sin(a);
    return mat2(c, s, -s, c);
}

// Simple noise function using built-in noise approximation
float noise(float t) {
...
float noise(in vec3 p) {
    ...
    float n000 = fract(sin(dot(ip, vec3(12.9898, 78.233, 37.719))) * 43758.5453);
    float n100 = fract(sin(dot(ip + vec3(1,0,0), vec3(12.9898, 78.233, 37.719))) * 43758.5453);
    float n010 = fract(sin(dot(ip + vec3(0,1,0), vec3(12.9898, 78.233, 37.719))) * 43758.5453);
    float n110 = fract(sin(dot(ip + vec3(1,1,0), vec3(12.9898, 78.233, 37.719))) * 43758.5453);
    float n001 = fract(sin(dot(ip + vec3(0,0,1), vec3(12.9898, 78.233, 37.719))) * 43758.5453);
    float n101 = fract(sin(dot(ip + vec3(1,0,1), vec3(12.9898, 78.233, 37.719))) * 43758.5453);
    float n011 = fract(sin(dot(ip + vec3(0,1,1), vec3(12.9898, 78.233, 37.719))) * 43758.5453);
    float n111 = fract(sin(dot(ip + vec3(1,1,1), vec3(12.9898, 78.233, 37.719))) * 43758.5453);
...
vec4 marchV(in vec3 ro, in vec3 rd, in float t, in vec3 bgc) {
    ...
    for (int i = 0; i < 150; i++) {
        ...
        col.rgb += clamp((1.0 - den * 6.0) + pos.y * 0.13 + 0.55, 0.0, 1.0) * 0.35 * mix(bgc, vec3(1), 0.7); // Fringes
        col += clamp(den * pos.y * 0.15, -0.02, 0.0); // Depth occlusion
        col *= smoothstep(0.2 + moy * 0.05, 0.0, mapV(pos + 1.0 * lgt)) * 0.85 + 0.15; // Shadows
