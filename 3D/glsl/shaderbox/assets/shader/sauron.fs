...
vec3 color(float grad) {
    // Convert mouse position to the range used in original shader
    // u_mouse_pos is normalized [0,1], we need to simulate iMouse behavior
    float mouse_pressed = length(u_mouse_pos - vec2(0.5)) > 0.1 ? 1.0 : 0.0;
    float m2 = mouse_pressed < 0.5 ? 1.15 : u_mouse_pos.y * 3.0;
...
void main() {
    // Convert from UV coordinates to fragment coordinates
    vec2 fragCoord = vs_uv * u_resolution;
    
    // Convert mouse position to the range used in original shader
    float mouse_pressed = length(u_mouse_pos - vec2(0.5)) > 0.1 ? 1.0 : 0.0;
    float m1 = mouse_pressed < 0.5 ? 3.6 : u_mouse_pos.x * 5.0;
    
    float t = u_time;
    vec2 uv = fragCoord / u_resolution.yy;
    float ff = 1.0 - uv.y;
