#version 330
// sun.fs 
// TODO: voronoi patterns with natural solar activity
// https://bgr.com/science/ultra-close-up-photos-show-the-sun-like-youve-never-seen-it-before/
...
float fbmDetailed(vec2 p, int octaves) {
    float value = 0.0;
    float amplitude = 0.5;
    float frequency = 1.0;
...
// Natural solar activity simulation
float solarActivity(float time) {
    // Combine multiple frequencies for natural variation
    float slow = fbmDetailed(vec2(time * 0.02, 0.0), 3) * 0.4;
    float medium = fbmDetailed(vec2(time * 0.08, 1.5), 2) * 0.3;
    float fast = fbmDetailed(vec2(time * 0.15, 3.7), 2) * 0.2;
    
    // Add some irregular bursts
    float burst = smoothstep(0.7, 0.9, fbmDetailed(vec2(time * 0.05, 2.3), 2)) * 0.4;
    
    return 0.7 + slow + medium + fast + burst;
}

// Realistic glow intensity with natural variation
float glowVariation(vec2 uv, float time) {
    // Multiple layers of variation at different scales
    float largeScale = fbmDetailed(uv * 0.5 + time * 0.01, 3) * 0.3;
    float mediumScale = fbmDetailed(uv * 1.5 + time * 0.03, 2) * 0.2;
    float smallScale = fbmDetailed(uv * 4.0 + time * 0.08, 2) * 0.15;
    
    // Combine with base intensity
    return 0.8 + largeScale + mediumScale + smallScale;
}
...
void main() {
    ...
    // Natural distance-based glow with realistic variation
    vec2 centerUV = (vs_uv - 0.5) * 2.0;
    float glowDistance = length(centerUV);
    float glowIntensity = max(0.0, 1.0 - glowDistance);
    
    // Apply natural variation to the glow
    float naturalGlow = glowVariation(centerUV, u_time);
    float globalActivity = solarActivity(u_time);
    
    // Multi-layered glow with different falloffs
    float innerGlow = pow(glowIntensity, 0.8) * 2.0;
    float outerGlow = pow(glowIntensity, 2.5) * 1.5;
    float coronaGlow = pow(glowIntensity, 4.0) * 3.0;
    
    // Combine glows with natural variation
    vec4 distanceGlow = vec4(1.0, 0.4, 0.1, 1.0) * 
                       (innerGlow + outerGlow * 0.7 + coronaGlow * 0.4) * 
                       naturalGlow * globalActivity;
    ...
    if (dist < 0.0) {
        ...
        float surface1 = fbmDetailed(animatedUV1 * 6.0, 4);
        float surface2 = fbmDetailed(animatedUV2 * 4.0, 3);
        float surface3 = fbmDetailed(animatedUV3 * 8.0, 2);
        
        // Combine surfaces for more complex patterns
        float combinedSurface = surface1 * 0.5 + surface2 * 0.3 + surface3 * 0.2;
        
        // More detailed granulation with natural variation
        float granulation = fbmDetailed(animatedUV1 * 15.0 + fbmDetailed(vec2(u_time * 0.05), 2) * 0.1, 3) * 0.3;
        
        // Enhanced solar flares with natural intensity variation
        float flareActivity = fbmDetailed(vec2(u_time * 0.03, 2.7), 3) * 0.5 + 0.5;
        float flare = 0.0;
        float flareNoise = fbmDetailed(animatedUV2 * 3.0 + u_time * 0.15, 3);
        if (flareNoise > 0.65) {
            flare = pow((flareNoise - 0.65) / 0.35, 2.0) * 0.6 * flareActivity;
        }
        
        // Add magnetic field lines with natural variation
        float magneticVariation = fbmDetailed(vec2(u_time * 0.02), 2) * 0.5 + 0.5;
        float magneticLines = sin(sphereUV.y * PI * 12.0 + u_time * magneticVariation) * 0.1;
        magneticLines *= smoothstep(0.3, 0.7, abs(sin(sphereUV.x * PI * 8.0)));
        
        // Sunspots with slow evolution
        float sunspots = fbmDetailed(sphereUV * 3.0 + u_time * 0.005, 4);
        ...
        // Enhanced edge glow with natural pulsing
        float edgeGlow = pow(1.0 - viewAngle, 2.5);
        float coronaPulse = fbmDetailed(vec2(u_time * 0.04, 1.9), 3) * 0.2 + 0.8;
        vec3 coronaColor = vec3(1.0, 0.6, 0.3) * edgeGlow * 0.7 * coronaPulse;
        
        // Add some chromatic aberration at the edges for realism
        float chromaticShift = edgeGlow * 0.1;
        coronaColor.r += chromaticShift;
        coronaColor.b -= chromaticShift * 0.5;
        
        // Combine surface and corona
        vec3 finalColor = surfaceColor + coronaColor;
        
        // Apply global solar activity variation
        finalColor *= globalActivity;
