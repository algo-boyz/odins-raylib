...
vec4 render(in vec3 ro, in vec3 rd) {
    ...
    for(int i = 0; i < 70; i++) {
        ...
        float dif = clamp((dn - map(pos * vec3(1.2) + 0.3)) / 8., 0.01, 1.);
        float dif2 = clamp((dn - map(pos * vec3(1.1) + 0.7)) / 6., 0.01, 1.);
        col.xyz *= vec3(0.01, 0.01, 0.01) + vec3(0.14, 0.12, 0.1) * dif + vec3(0.15, 0.12, 0.1) * dif2;
        
        rez = rez + col * (1. - rez.a);
        t += clamp(0.12 - den * 0.1, 0.05, 0.15);
    }
