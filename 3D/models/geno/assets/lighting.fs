...
void main()
{
    ...
    vec3 fragNormal = normalAndGlossiness.xyz * 2.0 - 1.0;
    vec3 albedo = colorAndSpec.xyz;
    float specularity = colorAndSpec.w;
    float glossiness = normalAndGlossiness.w * 100.0;
    ...
    finalColor = vec4(ToGamma(exposure * final), 1.0);
    //finalColor = vec4(ToGamma(vec3(ambientShadow, ambientShadow, ambientShadow)), 1.0);
    //finalColor = vec4(ToGamma(vec3(sunShadow, sunShadow, sunShadow)), 1.0);
    //finalColor = vec4(ToGamma(vec3(specular, specular, specular)), 1.0);
