...
static inline Frustum FrustumFromCameraMatrices(Matrix proj, Matrix m) {
    Matrix planes = { 0 };
    planes.m0 = m.m0 * proj.m0 + m.m1 * proj.m4 + m.m2 * proj.m8 + m.m3 * proj.m12;
    planes.m1 = m.m0 * proj.m1 + m.m1 * proj.m5 + m.m2 * proj.m9 + m.m3 * proj.m13;
    planes.m2 = m.m0 * proj.m2 + m.m1 * proj.m6 + m.m2 * proj.m10 + m.m3 * proj.m14;
    planes.m3 = m.m0 * proj.m3 + m.m1 * proj.m7 + m.m2 * proj.m11 + m.m3 * proj.m15;
    planes.m4 = m.m4 * proj.m0 + m.m5 * proj.m4 + m.m6 * proj.m8 + m.m7 * proj.m12;
    planes.m5 = m.m4 * proj.m1 + m.m5 * proj.m5 + m.m6 * proj.m9 + m.m7 * proj.m13;
    planes.m6 = m.m4 * proj.m2 + m.m5 * proj.m6 + m.m6 * proj.m10 + m.m7 * proj.m14;
    planes.m7 = m.m4 * proj.m3 + m.m5 * proj.m7 + m.m6 * proj.m11 + m.m7 * proj.m15;
    planes.m8 = m.m8 * proj.m0 + m.m9 * proj.m4 + m.m10 * proj.m8 + m.m11 * proj.m12;
    planes.m9 = m.m8 * proj.m1 + m.m9 * proj.m5 + m.m10 * proj.m9 + m.m11 * proj.m13;
    planes.m10 = m.m8 * proj.m2 + m.m9 * proj.m6 + m.m10 * proj.m10 + m.m11 * proj.m14;
    planes.m11 = m.m8 * proj.m3 + m.m9 * proj.m7 + m.m10 * proj.m11 + m.m11 * proj.m15;
    planes.m12 = m.m12 * proj.m0 + m.m13 * proj.m4 + m.m14 * proj.m8 + m.m15 * proj.m12;
    planes.m13 = m.m12 * proj.m1 + m.m13 * proj.m5 + m.m14 * proj.m9 + m.m15 * proj.m13;
    planes.m14 = m.m12 * proj.m2 + m.m13 * proj.m6 + m.m14 * proj.m10 + m.m15 * proj.m14;
    planes.m15 = m.m12 * proj.m3 + m.m13 * proj.m7 + m.m14 * proj.m11 + m.m15 * proj.m15;
...
// BVH File Data
// Types of "channels" that are possible
enum {
...
// Data associated with a single "joint"
typedef struct {
    int parent;
    char* name;
    Vector3 offset;
...
// Data structure matching whats in the BVH file
typedef struct {
    // Hierarchy Data
    int jointCount;
    BVHJointData* joints;
...
// Parse "channels" part of the BVH
...
// Sampled pose as joint transforms
typedef struct {
    int jointCount;
    int* parents;
    bool* endSite;
...
// Character Data
// Max characters allowed in scene
enum {
    CHARACTERS_MAX = 6,
};
typedef struct {
...
// Init CharacterData to a safe state
...
static bool CharacterDataLoadFromFile(CharacterData* data, const char* path, char* errMsg, int errMsgSize) {
...
// TODO load from 'assets/capsule.fs' file instead of tinyobj embedding
);

// Fragment Shader
static const char* shaderFS = GLSL(
// TODO load from 'assets/capsule.fs' file
);

// Structure containing all uniform indices for the shader
typedef struct {
    int isCapsule;
    int capsulePosition;
    int capsuleRotation;
...
int main(int argc, char** argv) {
    ...
    // TODO skip the File Dialog for the ODIN translation
    app.fileDialogState = InitGuiWindowFileDialog(GetWorkingDirectory());
    // Reset Error Message
    app.errMsg[0] = '\0';
    // Load any files given as cmd line args
    ...
    if (app.characterData.count > 0) {
        ...
        char title[512];
        snprintf(title, 512, "%s - BVH", app.characterData.filePaths[app.characterData.active]);
