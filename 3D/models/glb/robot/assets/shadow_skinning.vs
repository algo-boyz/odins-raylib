// shadow_skinning.vs - Combined vertex shader
#version 330
...
void main()
{
    ...
    // Send vertex attributes to fragment shader (using skinned values)
    fragPosition = vec3(matModel * skinnedPosition);
    fragTexCoord = vertexTexCoord;
    fragColor = vertexColor;
    fragNormal = normalize(vec3(matNormal * skinnedNormal));

    // Calculate final vertex position (using skinned position)
