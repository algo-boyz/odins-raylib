...
// Springs
// based on articles by Daniel Holden https://theorangeduck.com/page/spring-roll-call
...
// dt - deltaTime
exp_decay :: proc "contextless" (a, b, decay, dt: $T) -> T where intrinsics.type_is_numeric(T) {
    return b+(a-b)*math.exp(-decay*dt)
}

// remap takes a value between iMin and iMax and remaps it to a value between oMin and oMax
remap :: proc "contextless" (iMin, iMax, oMin, oMax, val: $T) -> T where intrinsics.type_is_numeric(T) {
    t := inv_lerp(iMin, iMax, val)
    // lerp returns a blend between a and b, based on a fraction t
    return math.lerp(oMin, oMax, t)
}

eerp :: proc "contextless" (x, y, a: $T) -> T where intrinsics.type_is_numeric(T) {
    return math.pow(x, (1.0 - a)) * math.pow(y, a)
}

eerp_alt :: proc "contextless" (x, y, a: $T) -> T where intrinsics.type_is_numeric(T) {
    return x * math.pow(y / x, a)
}

lerp_alt :: proc "contextless" (x, y, a: $T) -> T where intrinsics.type_is_numeric(T) {
    return x + (y - x) * a
}

// inverse lerp returns a fraction t, based on a value between a and b
inv_lerp :: proc "contextless" (a, b, val: $T) -> T where intrinsics.type_is_numeric(T) {
    return (val - a) / (b - a)
}
...
fast_negexp :: proc "contextless" (x: $T) -> T where intrinsics.type_is_numeric(T) {
...
fast_atan :: proc "contextless" (x: $T) -> T where intrinsics.type_is_numeric(T) {
    z := abs(x)
    w := z > 1.0 ? 1.0 / z : z
...
square :: proc "contextless" (x: $T) -> T where intrinsics.type_is_numeric(T) {
    return x * x
}

frequency_to_stiffness :: proc "contextless" (frequency: $T) -> T where intrinsics.type_is_numeric(T) {
    return square(2.0 * math.PI * frequency)
}

stiffness_to_frequency :: proc "contextless" (stiffness: $T) -> T where intrinsics.type_is_numeric(T) {
    return math.sqrt(stiffness) / (2.0 * math.PI)
}

halflife_to_damping :: proc "contextless" (halflife: $T, eps: T = 1e-5) -> T where intrinsics.type_is_numeric(T) {
    return (4.0 * 0.69314718056) / (halflife + eps)
}

damping_to_halflife :: proc "contextless" (damping: $T, eps: T = 1e-5) -> T where intrinsics.type_is_numeric(T) {
    return (4.0 * 0.69314718056) / (damping + eps)
}

critical_halflife :: proc "contextless" (frequency: $T) -> T where intrinsics.type_is_numeric(T) {
    return damping_to_halflife(math.sqrt(frequency_to_stiffness(frequency) * 4.0))
}

critical_frequency :: proc "contextless" (halflife: $T) -> T where intrinsics.type_is_numeric(T) {
...
damping_ratio_to_stiffness :: proc "contextless" (ratio, damping: $T) -> T where intrinsics.type_is_numeric(T) {
    return square(damping / (ratio * 2.0))
}

damping_ratio_to_damping :: proc "contextless" (ratio, stiffness: $T) -> T where intrinsics.type_is_numeric(T) {
...
spring_character_predict :: proc "contextless" (
    ...
    for i := 0; i < count; i += 1 {
        spring_character_update(&px[i], &pv[i], &pa[i], v_goal, halflife, T(i) * dt)
    }
}
...
tracking_spring_update_exact :: proc "contextless" (
    x, v: ^$T,
    x_goal, v_goal, a_goal: T,
    x_gain, v_gain, a_gain: T,
    dt, gain_dt: T,
) where intrinsics.type_is_numeric(T) {
