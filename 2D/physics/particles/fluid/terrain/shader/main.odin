...
noise2d :: proc(x, y: f32) -> f32 {
	return f32(noise.perlin_noise_2d(&perl, f64(x), f64(y))  * 0.5 + 0.55)
}
...
// Update the simulation data texture from CPU arrays
update_sim_texture :: proc(bed, water, sediment: [dynamic][dynamic]f32) {
    idx := 0
    for y in 0..<N {
        for x in 0..<N {
            // Pack data into RGBA float buffer
            sim_data_buffer[idx + 0] = bed[y][x]      // R = Bed Height
            sim_data_buffer[idx + 1] = water[y][x]    // G = Water Height
            sim_data_buffer[idx + 2] = sediment[y][x] // B = Sediment Amount
            sim_data_buffer[idx + 3] = 0.0            // A = Unused (or other data like velocity magnitude)
            idx += 4
        }
    }
    // Upload the buffer data to the GPU texture
    rl.UpdateTexture(sim_texture, raw_data(sim_data_buffer))
}

draw_simulation :: proc(camera: rl.Camera2D) {
	rl.BeginMode2D(camera)
    rl.BeginShaderMode(sim_shader)
    // Set shader uniforms
    rl.SetShaderValue(sim_shader, simDataTexLoc, &sim_texture.id, .SAMPLER2D) // Pass texture ID via address
    tex_size := [2]f32{f32(N), f32(N)}
    rl.SetShaderValue(sim_shader, textureSizeLoc, &tex_size, .VEC2)
    rl.SetShaderValue(sim_shader, lightDirectionLoc, &LIGHT_DIRECTION_UNIFORM, .VEC3)
    max_bed : f32 = MAX_BED_HEIGHT_FOR_COLOR
    max_water : f32 = MAX_WATER_HEIGHT_FOR_COLOR
    max_sediment : f32 = MAX_SEDIMENT_FOR_COLOR
    rl.SetShaderValue(sim_shader, maxBedHeightLoc, &max_bed, .FLOAT)
    rl.SetShaderValue(sim_shader, maxWaterHeightLoc, &max_water, .FLOAT)
    rl.SetShaderValue(sim_shader, maxSedimentLoc, &max_sediment, .FLOAT)

    // Define source and destination rectangles for drawing the texture
    sourceRec := rl.Rectangle{0, 0, f32(sim_texture.width), f32(sim_texture.height)}
    destRec := Simulation_Area // Draw texture covering the simulation area world coordinates
    origin := rl.Vector2{0, 0}
    rotation: f32 = 0.0
    tint := rl.WHITE // Can apply a global tint here if needed

    // Draw the texture using the shader
    rl.DrawTexturePro(sim_texture, sourceRec, destRec, origin, rotation, tint)
...
main :: proc() {
    ...
    // Load shaders
    sim_shader = rl.LoadShader("sim_display.vs", "sim_display.fs")
    if sim_shader.id == 0 {
        fmt.eprintln("Error loading shader!")
        // Handle error appropriately
    }

    // Get uniform locations (check for -1 if uniform not found)
    simDataTexLoc     = rl.GetShaderLocation(sim_shader, "simDataTexture")
    textureSizeLoc    = rl.GetShaderLocation(sim_shader, "textureSize")
    lightDirectionLoc = rl.GetShaderLocation(sim_shader, "lightDirection")
    maxBedHeightLoc   = rl.GetShaderLocation(sim_shader, "maxBedHeightColor")
    maxWaterHeightLoc = rl.GetShaderLocation(sim_shader, "maxWaterHeightColor")
    maxSedimentLoc    = rl.GetShaderLocation(sim_shader, "maxSedimentColor")

    // Prepare light direction uniform (normalize)
    light_sim := rl.Vector2Normalize(rl.Vector2{-1.0, -1.0}) // Light from top-left XY
    LIGHT_DIRECTION_UNIFORM = {light_sim.x, light_sim.y, 0.5} // Add some Z component (pointing slightly down)
    // Normalize the 3D vector
    light_len := math.sqrt(LIGHT_DIRECTION_UNIFORM[0]*LIGHT_DIRECTION_UNIFORM[0] + LIGHT_DIRECTION_UNIFORM[1]*LIGHT_DIRECTION_UNIFORM[1] + LIGHT_DIRECTION_UNIFORM[2]*LIGHT_DIRECTION_UNIFORM[2])
    if light_len > 1e-6 {
        LIGHT_DIRECTION_UNIFORM[0] /= light_len
        LIGHT_DIRECTION_UNIFORM[1] /= light_len
        LIGHT_DIRECTION_UNIFORM[2] /= light_len
    }


    // Allocate buffer for texture data (RGBA Float32)
    sim_data_buffer = make([]f32, N * N * 4)

    // Create the texture - Load empty texture with the correct float format
    // We need raw OpenGL for this as Raylib LoadTextureFromImage expects specific formats
    // Ensure rlgl is initialized (happens implicitly with InitWindow)
    tex_id := rlgl.LoadTexture(nil, N, N, cast(i32)rl.PixelFormat.UNCOMPRESSED_R32G32B32A32, 1)
    if tex_id == 0 {
         fmt.eprintln("Error creating float texture!")
         // Handle error
    }
    sim_texture = rl.Texture2D {
        id = tex_id,
        ...
        height = N,
        mipmaps = 1,
        format = .UNCOMPRESSED_R32G32B32A32, // Store the format
    }

    // Set texture parameters (important!)
    rl.SetTextureWrap(sim_texture, .CLAMP)
    // rl.SetTextureFilter(sim_texture, .NEAREST) // Use NEAREST for sharp grid cells


    // Initial texture upload (optional, can be black or initial state)
	...
	delete(new_sediment)
	delete(particles)
    rl.UnloadShader(sim_shader)
    rl.UnloadTexture(sim_texture)
	rl.CloseWindow()
}
