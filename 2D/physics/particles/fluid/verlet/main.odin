...
particle_update_worker :: proc(arg: rawptr) {
    ...
    for i in work.start_idx..<work.end_idx {
        ...
        if current_mode == .FLUID {
            ...
            fluid_bounds(particle, work.screen_width, work.screen_height)
        } else {
            constrain_to_bounds(particle, work.screen_width, work.screen_height)
        }
    }
}
...
update_particles_threaded :: proc(delta_time: f32) {
    ...
    for i in 0..<THREAD_COUNT {
        ...
        work_items[i].end_idx = min((i + 1) * chunk_size, particle_count)
        work_items[i].delta_time = delta_time
        work_items[i].screen_width = int(screen_width)
        work_items[i].screen_height = int(screen_height)
