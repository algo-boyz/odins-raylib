...
// Thread work data structures
ParticleUpdateWork :: struct {
    start_idx: int,
    end_idx: int,
    delta_time: f32,
    WIDTH: int,
...
// Thread pool
thread_pool: [THREAD_COUNT]fibr.Thread
...
pick_force: rl.Vector2 = {-150.0, -150.0}
dragging: bool = false
particle_color: rl.Color = rl.RAYWHITE
bg_color: rl.Color = rl.BLACK
mu_ctx: mu.Context
...
initialize_particles :: proc(count: int, color: rl.Color) {
    clear(&particles)
    for i in 0..<count {
        p := Particle{
...
velocity_to_color :: proc(velocity: rl.Vector2, max_vel: f32) -> rl.Color {
...
// Worker function for collision detection
collision_worker :: proc(arg: rawptr) {
    work := cast(^CollisionWork)arg
    
    for i in work.start_idx..<work.end_idx {
...
resolve_grid_collisions_threaded :: proc() {
    particle_count := len(particles)
    if particle_count == 0 do return
    
    chunk_size := max(1, particle_count / THREAD_COUNT)
    work_items: [THREAD_COUNT]CollisionWork
    
    for i in 0..<THREAD_COUNT {
        work_items[i].start_idx = i * chunk_size
        work_items[i].end_idx = min((i + 1) * chunk_size, particle_count)
        
        if work_items[i].start_idx < particle_count {
            fibr.spawn(&thread_pool[i], collision_worker, &work_items[i])
        }
    }
    ...
    for i in 0..<THREAD_COUNT {
        if work_items[i].start_idx < particle_count {
            fibr.join(&thread_pool[i])
        }
    }
}

constrain_to_bounds :: proc(p: ^Particle, WIDTH, HEIGHT: int) {
    if p.pos.x < p.radius {
        p.pos.x = p.radius
    } else if p.pos.x > f32(WIDTH) - p.radius {
        p.pos.x = f32(WIDTH) - p.radius
    }
    if p.pos.y < p.radius {
        p.pos.y = p.radius
    } else if p.pos.y > f32(HEIGHT) - p.radius {
        p.pos.y = f32(HEIGHT) - p.radius
    }
}

fluid_bounds :: proc(p: ^Particle, WIDTH, HEIGHT: int) {
    min_x := p.radius
    max_x := f32(WIDTH) - p.radius
    min_y := p.radius
    max_y := f32(HEIGHT) - p.radius
...
// Worker function for viscosity calculation
viscosity_worker :: proc(arg: rawptr) {
    work := cast(^SPHWork)arg
        ...
        for j in 0..<len(particles) {
            ...
            if distance < h {
                ...
                if radial_velocity > 0 {
                    q := distance / h
                    impulse_magnitude := work.delta_time * (1 - q) * (sigma * radial_velocity + beta * radial_velocity * radial_velocity)
                    impulse := direction * impulse_magnitude
                    
                    // Note: This introduces race conditions, but for visual simulation it's acceptable
                    sync.mutex_lock(&work_mutex)
                    pi.velocity = pi.velocity - impulse * 0.5
                    pj.velocity = pj.velocity + impulse * 0.5
                    sync.mutex_unlock(&work_mutex)
                }
            }
        }
    }
...
apply_viscosity_threaded :: proc(delta_time: f32) {
    particle_count := len(particles)
    if particle_count == 0 do return
    
    chunk_size := max(1, particle_count / THREAD_COUNT)
    work_items: [THREAD_COUNT]SPHWork
    
    for i in 0..<THREAD_COUNT {
        work_items[i].start_idx = i * chunk_size
        work_items[i].end_idx = min((i + 1) * chunk_size, particle_count)
        work_items[i].delta_time = delta_time
        
        if work_items[i].start_idx < particle_count {
            fibr.spawn(&thread_pool[i], viscosity_worker, &work_items[i])
        }
    }
...
// Worker function for density calculation
density_worker :: proc(arg: rawptr) {
    work := cast(^SPHWork)arg
    ...
    for i in work.start_idx..<work.end_idx {
        ...
        neighbor_direction: [dynamic]rl.Vector2
        ...
        for n in 0..<len(neighbors) {
            ...
            displacement_contribution := direction * (work.delta_time * work.delta_time) * (pressure * closeness + near_pressure * closeness * closeness)
...
double_density_relaxation_threaded :: proc(delta_time: f32) {
    particle_count := len(particles)
    if particle_count == 0 do return
    ...
    for i in 0..<THREAD_COUNT {
        ...
        if work_items[i].start_idx < particle_count {
            fibr.spawn(&thread_pool[i], density_worker, &work_items[i])
        }
    }
...
// Worker function for particle updates
particle_update_worker :: proc(arg: rawptr) {
    work := cast(^ParticleUpdateWork)arg
    ...
    for i in work.start_idx..<work.end_idx {
        ...
        verlet_integration(particle, work.delta_time)
        
        speed := linalg.length(particle.velocity)
        if speed > max_velocity {
            // Note: Race condition here, but for visual purposes it's acceptable
            max_velocity = speed
        }
        
        if current_mode == .FLUID {
            particle.color = velocity_to_color(particle.velocity, max_velocity)
            fluid_bounds(particle, work.WIDTH, work.HEIGHT)
        } else {
            constrain_to_bounds(particle, work.WIDTH, work.HEIGHT)
        }
    }
}

update_particles_threaded :: proc(delta_time: f32) {
    ...
    chunk_size := max(1, particle_count / THREAD_COUNT)
    work_items: [THREAD_COUNT]ParticleUpdateWork
    
    for i in 0..<THREAD_COUNT {
        work_items[i].start_idx = i * chunk_size
        work_items[i].end_idx = min((i + 1) * chunk_size, particle_count)
        work_items[i].delta_time = delta_time
        work_items[i].WIDTH = int(WIDTH)
        work_items[i].HEIGHT = int(HEIGHT)
        
        if work_items[i].start_idx < particle_count {
            fibr.spawn(&thread_pool[i], particle_update_worker, &work_items[i])
        }
    }
...
update_sph_particles :: proc(delta_time: f32) {
    apply_viscosity_threaded(delta_time)
    update_particles_threaded(delta_time)
    double_density_relaxation_threaded(delta_time)
}

update_verlet_particles :: proc(delta_time: f32) {
    update_particles_threaded(delta_time)
    assign_particles_to_grid()
    resolve_grid_collisions_threaded()
}
...
spawn_particle :: proc(position: rl.Vector2, radius: f32, color: rl.Color) {
    p := Particle{
        pos = position,
        old_pos = position,
        accel = {0, 0},
...
push_particles :: proc(position: rl.Vector2, force: f32) {
    for &particle in particles {
...
pick_up_particles :: proc(position: rl.Vector2, radius: f32, force: rl.Vector2) {
    for &particle in particles {
...
draw_ui :: proc(ctx: ^mu.Context) {
    if mu.window(ctx, "Simulation Settings", {40, 40, 300, 450}) {
        // Simulation mode
        ...
        // Performance info
        mu.label(ctx, fmt.tprintf("Threads: %d", THREAD_COUNT))
...
main :: proc() {
    rl.InitWindow(WIDTH, HEIGHT, "Multithreaded Verlet Integration")
    defer rl.CloseWindow()
    
    // Init mutex
    defer free(&work_mutex)
    ...
    // Init microui with required callbacks
    mu.init(&mu_ctx)
    ...
    for !rl.WindowShouldClose() {
        // Update physics with multithreading
        switch current_mode {
        case .VERLET:
            update_verlet_particles(0.016)  // Reduced timestep for stability
        case .FLUID:
            update_sph_particles(0.016)
        }
        ...
        // Show thread count
        thread_text := fmt.tprintf("Threads: %d", THREAD_COUNT)
        rl.DrawText(cstring(raw_data(thread_text)), WIDTH - text_width - 10, 35, 20, rl.DARKGRAY)
    }
}