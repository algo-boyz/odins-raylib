...
State :: struct {
    ...
    reveal_mode: bool,
    pixel_influence: [GRID_WIDTH][GRID_HEIGHT]f32, // Track influence grid
    dissolve_mode: bool, // New dissolving reality mode
}
...
update_influence :: proc() {
    // Decay influence over time
    for x in 0..<GRID_WIDTH {
        for y in 0..<GRID_HEIGHT {
            state.pixel_influence[x][y] *= 0.95 // Fade out
        }
    }
    
    // Add influence where characters are
    for i in 0..<state.cnt {
        stream := &state.streams[i]
        if stream.active {
            for j in 0..<stream.length {
                char_y := stream.y - f32(j) * 20
                grid_x := int(stream.x / GRID_SIZE)
                grid_y := int(char_y / GRID_SIZE)
                
                // Check bounds and add influence with falloff
                if grid_x >= 0 && grid_x < GRID_WIDTH && grid_y >= 0 && grid_y < GRID_HEIGHT {
                    fade_factor := 1.0 - (f32(j) / f32(stream.length))
                    influence := fade_factor * 1.0
                    
                    // Add influence to current grid cell
                    state.pixel_influence[grid_x][grid_y] = max(state.pixel_influence[grid_x][grid_y], influence)
                    
                    // Add influence to neighboring cells for smoother effect
                    for dx in -1..=1 {
                        for dy in -1..=1 {
                            nx := grid_x + dx
                            ny := grid_y + dy
                            if nx >= 0 && nx < GRID_WIDTH && ny >= 0 && ny < GRID_HEIGHT {
                                neighbor_influence := influence * 0.6 // Reduced influence for neighbors
                                state.pixel_influence[nx][ny] = max(state.pixel_influence[nx][ny], neighbor_influence)
                            }
                        }
                    }
                }
...
update_game :: proc() {
    ...
    // Toggle dissolve mode
    if rl.IsKeyPressed(.D) {
        state.dissolve_mode = !state.dissolve_mode
    }
}
...
draw_dissolving_background :: proc() {
    // Draw the background with dissolving effect based on pixel influence
    for x in 0..<GRID_WIDTH {
        for y in 0..<GRID_HEIGHT {
            influence := state.pixel_influence[x][y]
            
            // Calculate alpha based on influence (higher influence = more visible background)
            alpha := u8(255 * influence)
            
            if alpha > 5 { // Only draw if there's meaningful influence
                src_rect := rl.Rectangle{
                    x = f32(x * GRID_SIZE) * f32(state.bg.width) / WIDTH,
                    y = f32(y * GRID_SIZE) * f32(state.bg.height) / HEIGHT,
                    width = f32(GRID_SIZE) * f32(state.bg.width) / WIDTH,
                    height = f32(GRID_SIZE) * f32(state.bg.height) / HEIGHT,
                }
                ...
                // Draw this grid cell with calculated alpha
                rl.DrawTexturePro(
                    state.bg,
                    src_rect,
                    dest_rect,
...
draw_game :: proc() {
    if state.reveal_mode {
    ...
    } else if state.dissolve_mode {
        // DISSOLVE MODE: Background fades in/out based on character presence
        rl.BeginDrawing()
        rl.ClearBackground(rl.BLACK)
        rl.DrawTexturePro(
            state.bg, 
        ...
        // Draw dissolving background
        draw_dissolving_background()
        
        // Draw characters on top
        draw_characters_normal()
        
        // Draw UI text
        rl.DrawText("Press D to toggle dissolve mode", 10, 10, 20, rl.WHITE)
        rl.DrawText("Dissolve Mode: ON", 10, 35, 20, rl.YELLOW)
        ...
        // Draw UI text
        rl.DrawText("Press T to toggle reveal mode", 10, 10, 20, rl.WHITE)
        rl.DrawText("Press D to toggle dissolve mode", 10, 35, 20, rl.WHITE)
        rl.DrawText("Normal Mode", 10, 60, 20, rl.WHITE)
    }
    ...
    rl.DrawText("D: Toggle Dissolve Mode", 10, HEIGHT - 40, 16, rl.WHITE)
...
main :: proc() {
    ...
    // Init
    state.reveal_mode = false
    state.dissolve_mode = false
    init_streams()
