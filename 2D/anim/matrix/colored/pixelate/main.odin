...
// Get pixelated background color by sampling from pixel blocks
get_pixelated_background_color :: proc(x, y: f32) -> rl.Color {
    // Snap to pixel grid
    pixel_x := i32(x / f32(state.pixel_size)) * state.pixel_size
    pixel_y := i32(y / f32(state.pixel_size)) * state.pixel_size
    
    // Sample from the center of the pixel block
    sample_x := f32(pixel_x) + f32(state.pixel_size) / 2
    sample_y := f32(pixel_y) + f32(state.pixel_size) / 2
    
    return get_background_color(sample_x, sample_y)
}

draw_pixelated_blocks :: proc() {
    // Draw pixelated blocks around character streams
    for i in 0..<state.cnt {
        stream := &state.streams[i]
        ...
        if stream.active {
            for j in 0..<stream.length {
                ...
                if char_y >= -20 && char_y <= HEIGHT + 20 {
                    // Calculate area around character to pixelate
                    block_radius := state.pixel_size * 3 // How far around the character to pixelate
                    
                    start_x := i32(stream.x - f32(block_radius))
                    end_x := i32(stream.x + f32(block_radius))
                    start_y := i32(char_y - f32(block_radius))
                    end_y := i32(char_y + f32(block_radius))
                    
                    // Draw pixelated blocks in the area around the character
                    for px := start_x; px < end_x; px += state.pixel_size {
                        for py := start_y; py < end_y; py += state.pixel_size {
                            // Skip if outside screen bounds
                            if px < 0 || py < 0 || px >= WIDTH || py >= HEIGHT {
                                continue
                            }
                            
                            // Get the pixelated color for this block
                            block_color := get_pixelated_background_color(f32(px), f32(py))
                            
                            // Calculate fade factor based on distance from character
                            char_center_x := stream.x + 10
                            char_center_y := char_y + 10
                            block_center_x := f32(px) + f32(state.pixel_size) / 2
                            block_center_y := f32(py) + f32(state.pixel_size) / 2
                            
                            distance := ((char_center_x - block_center_x) * (char_center_x - block_center_x) + 
                                       (char_center_y - block_center_y) * (char_center_y - block_center_y))
                            max_distance := f32(block_radius * block_radius)
                            
                            // Fade factor based on distance and stream position
                            fade_factor := 1.0 - (distance / max_distance)
                            fade_factor *= 1.0 - (f32(j) / f32(stream.length)) // Stream fading
                            fade_factor = max(0, min(1, fade_factor))
                            
                            if fade_factor > 0.1 { // Only draw if visible enough
                                // Apply fade to the block color
                                pixelated_color := rl.Color{
                                    u8(f32(block_color.r) * fade_factor),
                                    u8(f32(block_color.g) * fade_factor),
                                    u8(f32(block_color.b) * fade_factor),
                                    u8(255 * fade_factor * 0.7), // Slightly transparent
                                }
                                
                                // Draw the pixelated block
                                rl.DrawRectangle(px, py, state.pixel_size, state.pixel_size, pixelated_color)
                            }
                        }
                    }
                }
...
update_game :: proc() {
    ...
    // Toggle pixelation mode
    if rl.IsKeyPressed(.P) {
        state.pixelation_mode = !state.pixelation_mode
    }
    
    // Adjust pixel size
    if rl.IsKeyPressed(.MINUS) {
        state.pixel_size = max(4, state.pixel_size - 2)
    }
    if rl.IsKeyPressed(.EQUAL) {
        state.pixel_size = min(20, state.pixel_size + 2)
    }
}
...
draw_game :: proc() {
    ...
    // Draw pixelated blocks if pixelation mode is enabled
    if state.pixelation_mode {
        draw_pixelated_blocks()
    }
    ...
    // Draw pixelation mode indicator
    pixelation_text :cstring = state.pixelation_mode ? "Pixelation: ON" : "Pixelation: OFF"
    pixelation_color := state.pixelation_mode ? rl.GREEN : rl.RED
    rl.DrawText(pixelation_text, 10, 60, 20, pixelation_color)
    
    if state.pixelation_mode {
        pixel_size_text := rl.TextFormat("Pixel Size: %d", state.pixel_size)
        rl.DrawText(pixel_size_text, 10, 85, 20, rl.WHITE)
    }
    ...
    // Draw controls
    rl.DrawText("Controls:", 10, HEIGHT - 140, 16, rl.WHITE)
    rl.DrawText("SPACE: Pause", 10, HEIGHT - 120, 16, rl.WHITE)
    rl.DrawText("J/K: Speed Down/Up", 10, HEIGHT - 100, 16, rl.WHITE)
    rl.DrawText("R: Random Colors", 10, HEIGHT - 80, 16, rl.WHITE)
    rl.DrawText("T: Toggle Reveal Mode", 10, HEIGHT - 60, 16, rl.WHITE)
    rl.DrawText("P: Toggle Pixelation", 10, HEIGHT - 40, 16, rl.WHITE)
    rl.DrawText("-/=: Pixel Size", 10, HEIGHT - 20, 16, rl.WHITE)
    rl.DrawText("1-0: Select Color", 10, HEIGHT - 0, 16, rl.WHITE)
...
main :: proc() {
    ...
    state.pixelation_mode = false // Start with pixelation off
    state.pixel_size = 8 // Default pixel size
    init_streams()
